...
现有项目已经具备较完整的简历解析、AI 简历分析、文字模拟面试、RAG 知识库、SSE 流式回答、Redis Stream 异步任务和 Docker 部署能力，适合作为 CareerAI 的功能底座。
...
| 语音模拟面试 | 已从当前路线移除 | 聚焦纯文字面试，降低调试成本和演示风险 |
...
| 用户数据隔离 | 大多数实体无 `userId` | 全链路补齐用户归属和越权访问测试 |
...
首版必须把这条链路走通。ASR/TTS 语音面试不纳入当前版本；日程和多模型管理可以保留为 P2 范围，避免冲淡求职业务主线。
...
- [x] 移除语音面试、ASR/TTS 设置和相关入口，减少首版导航噪音。
...
- 删除 ASR/TTS、WebRTC 和复杂语音优化，当前项目只保留纯文字求职练习链路。
