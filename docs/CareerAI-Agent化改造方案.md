...
> 范围说明（2026-07-16）：项目主线已经收束为“简历–JD–规划”和“自适应模拟面试”两个闭环，持久能力画像作为共享记忆。本文保留更完整的历史工具与审批设计供参考；当前产品范围、默认运行依赖和实施优先级以根目录 `README.md` 为准。
...
- [x] Java 首批 Tool 契约和内部接口：简历列表/详情、岗位详情、匹配任务/报告、改进计划。
- [x] Python 首批 Tool 使用 Pydantic 输入输出，并透传用户、Run 和 Step 上下文。
- [x] 首批 Tool 完成错误 `retryable` 分类、HTTP 契约测试和假服务测试。
...
- [x] Java 侧岗位匹配任务与简历改进计划写 Tool 支持 `Idempotency-Key`。
- [x] Python 图支持岗位匹配异步等待、checkpoint 和认证恢复轮询。
- [x] Python 图接入匹配报告与简历改进计划，写操作使用稳定幂等键。
